...
class VueJsLayoutPublishCommand extends PublishBaseCommand
{
    ...
    private function copyView()
    {
        ...
        foreach ($files as $stub => $blade) {
            $sourceFile = base_path('vendor/trungpv93/'.$templateType.'/templates/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($vueLayoutFiles as $stub => $blade) {
            $sourceFile = base_path('vendor/trungpv93/'.$templateType.'/templates/vuejs/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($baseRequestCustomFiles as $stub => $php) {
            $sourceFile = base_path('vendor/trungpv93/laravel-generator/templates/'.$stub);
        ...
        $sourceFile = base_path('vendor/trungpv93/laravel-generator/templates/vuejs/js/gulpfile.js');
        $destinationFile = base_path().'/gulpfile.js';
        $this->publishFile($sourceFile, $destinationFile, 'gulpfile.js');

        $sourceFile = base_path('vendor/trungpv93/laravel-generator/templates/vuejs/js/package.json');
